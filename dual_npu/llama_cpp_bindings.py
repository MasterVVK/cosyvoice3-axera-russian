...
All struct handling (llama_model_params, llama_context_params, llama_batch)
happens in C; Python only passes simple types and pointers.

Provides just enough API for CosyVoice3 TTS:
  - Load GGUF model
  - Feed custom embeddings
...
# --- Find and load llama_wrapper.so ---
_WRAPPER_PATHS = [
...
def _load_wrapper():
    # First load llama.cpp dependencies globally
...
_lib.wrapper_decode_embd.argtypes = [
    ctypes.c_void_p,                        # ctx
    ctypes.POINTER(ctypes.c_float),         # embd_data
    ctypes.c_int,                            # n_tokens
    ctypes.c_int,                            # n_embd
    ctypes.c_int,                            # pos_start
...
class LlamaCppModel:
    ...
    def __init__(self, model_path, n_ctx=512, n_threads=4):
        ...
        assert self.n_embd > 0, f"n_embd={self.n_embd}, model may not have loaded properly"

        self.ctx = _lib.wrapper_create_context(
            self.model, n_ctx, n_ctx, n_threads, 1
        )
        if not self.ctx:
    ...
    def get_hidden(self, embeddings, keep_history=0):
        ...
        Args:
            embeddings: numpy array [n_tokens, n_embd] or [n_embd] float32
            keep_history: if 0, clear KV cache first (prefill). if 1, append (decode step).

        Returns:
            numpy array [n_embd] float32 - hidden state of last token
        """
        if keep_history == 0:
        ...
        assert embeddings.shape[1] == self.n_embd, \
            f"Embedding dim mismatch: {embeddings.shape[1]} vs {self.n_embd}"
        ...
        if ret != 0:
            raise RuntimeError(f"wrapper_decode_embd failed with code {ret}")
    ...
    def state_get_size(self):
        """Get size of full context state (for info/logging)."""
        return _lib.wrapper_state_get_size(self.ctx)

    def state_save(self, path):
        """Save full context state (KV cache) to file. Returns 0 on success."""
    ...
    def state_load(self, path):
        """Load full context state (KV cache) from file. Returns 0 on success."""
