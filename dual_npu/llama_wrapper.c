...
int wrapper_state_save_file(struct llama_context* ctx, const char* path) {
    /* Save full context state (KV cache + internal state) to file.
     * We pass NULL/0 for tokens since we use custom embeddings. */
...
int wrapper_decode_embd(
    ...
    /* Set positions and output mask */
    for (int i = 0; i < n_tokens; i++) {
        batch.pos[i] = pos_start + i;
        batch.n_seq_id[i] = 1;
        batch.seq_id[i][0] = 0;
        batch.logits[i] = (i == n_tokens - 1) ? 1 : 0;  /* only last token */
    }
